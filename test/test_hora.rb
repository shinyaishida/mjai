...
class TC_Hora < Test::Unit::TestCase
  ...
  def new_hora(params)
    Hora.new(DEFAULT_PARAMS.merge(params))
  end

  def has_yaku?(yaku, params)
    new_hora(params).yakus.include?(yaku)
  end
  ...
  def test_yaku
    ...
    assert(has_yaku?([:jikaze, 1], {
                       tehais: Pai.parse_pais('123m456p777sP'),
                       furos: [Furo.new({ type: :pon, taken: Pai.new('S'), consumed: Pai.parse_pais('SS') })],
                       taken: Pai.new('P'),
                       hora_type: :ron
                     }))
    ...
    assert(has_yaku?([:sanshokudojun, 1], {
                       ...
                       furos: [Furo.new({ type: :chi, taken: Pai.new('2p'),
                                          consumed: Pai.parse_pais('34p') })],
                       taken: Pai.new('9s'),
                       hora_type: :ron
                     }))
