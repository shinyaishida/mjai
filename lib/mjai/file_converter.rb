...
module Mjai
  class FileConverter
    ...
    def dump_archive(archive_path, output_path, output_format)
      ...
      rescue Archive::UnsupportedArchiveError => e
        FileUtils.rm(output_path)
        warn(format('Skipping unsupported file %s (%s)', archive_path, e.message))
      end
    end
    ...
    def mjson_to_html(mjson_path, html_path)
      ...
      action_jsons = File.readlines(mjson_path).map { |s| s.chomp.gsub(%r{/}) { '\\/' } }
      ...
      Dir["#{res_dir}/css/*.css"] + Dir["#{res_dir}/js/*.js"].each do |src_path|
